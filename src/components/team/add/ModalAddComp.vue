...
const filteredStudents = computed(() => {
  const filtered = store.student.studentsClass.filter((s) =>
    s.user.name?.toLowerCase().includes(infoData.searchTerm.toLowerCase())
  )
      ...
      <div class="modalForm">
        <div class="inputs">
          ...
          <GradientSelect label="Turma do Integrante" v-model:option="infoData.turma">
            ...
            <option v-for="item in store.classInfo.classesInfo" :key="item.id" :value="item.id">
              {{ item.name }}
            </option>
          </GradientSelect>
          <div class="buttonAdd">
            <GradientSelect label="Integrante" v-model:option="infoData.selectedItem">
              <option disabled value="" v-if="!filteredStudents.length">
                Nenhum aluno disponível encontrado na turma.
              </option>
          ...
          <GradientSelect
            ...
            v-model:option="infoData.selectedLeader"
            @changeLeader="addLeader"
          >
            <option disabled value="">Selecione um líder:</option>
            <option v-for="item in infoData.selectedStudents" :key="item.id" :value="item.id">
              {{ item.user.name }}
            </option>
          </GradientSelect>
          <div class="listMembers">
            <div
              v-for="(item, index) in infoData.selectedStudents"
              :key="item.id"
              class="itemMember"
            >
              <p>{{ item.user.name }}</p>
