...
class AvaliationService {
  async getAvaliations() {
    try {
      ...
      toast.error(
        'Impossível verificar as avaliações! Verique suas credenciais ou tente mais tarde.'
      )
    }
  }
  ...
  async createAvaliation(avaliation) {
    try {
      ...
      toast.error(
        'Impossível criar avaliação! Verique suas credenciais, e se todos os campos foram preenchidos corretamente ou tente mais tarde.'
      )
    }
  }
  ...
  async updateAvaliation(avaliation) {
    try {
      ...
      toast.error(
        'Impossível editar avaliação! Verique suas credenciais, e se todos os campos foram editados corretamente ou tente mais tarde.'
      )
    }
  }
  ...
  async deleteAvaliation(id) {
    try {
      ...
      toast.error(
        'Impossível deletar avaliação! Verique suas credenciais, e se ela realmente existe ou tente mais tarde.'
      )
    }
  }

  async getTeamAvaliationsByAvaliator(evaluator, team) {
    const { data } = await api.get(`/avaliations/?team_id=${team}&evaluator_id=${evaluator}`)
    return data
  }
}
